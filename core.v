module core (
  ...
  fifo uart_rx_fifo(.clk(clk), .rst(rst),
    .write_en(rx_rdy),
    .write_data(rx_data),
    .read_en(cmd_rdy),
    .read_data(fifo_rd),
  ...
  always @(posedge clk) uart_ctsn <= fifo_watermark;
  ...
  reg rd_delay;
  always @(posedge clk) rd_delay <= cap_encoder_rdy && !cap_fifo_rd_empty;

  fifo #(.DEPTH(4096), .WIDTH(40)) capture_fifo(.clk(clk), .rst(rst),
    .write_en(capture_wr),
  ...
  encoder encoder(.clk(clk), .rst(rst),
    .capture_wr(rd_delay),
